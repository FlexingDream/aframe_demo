    ...
    for (let i =0;i<this.props.length; i++){
      ...
      blocks.push(
        <Entity position={startPosition} geometry={{primitive: 'box', width: 4, depth: blockLength}}  material={{opacity: i == 0 ? 1 : 0,color: Helper.getRandomColor()}}>
          {i == 0 ? '' : <Animation attribute="position" to={position} from={startPosition} ease="linear" dur="20000" begin={i*1000} />}
          {i == 0 ? '' : <Animation attribute="material.opacity" to="1" from="0" ease="linear" dur="20000" begin={i*1000}/>}
